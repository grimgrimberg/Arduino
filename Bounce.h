...
private:
  ...
  {
    //tm  Seconds;
    //const double T_s = time(NULL);
   // static int T_m = millis();
    double tv=time(NULL);
...
public:
  ...
  virtual void Draw()
  {
    Serial.print("hi im here inside the comment draw");
    if (_Fade)
    {
    ...
    {     
      ...
      size_t position = (size_t)(Height[i] * (_cLength - 1.0) / StartHeight);
      leds[position] = color[i];
       leds[position+1] = color[i];
      
      if (_bMirrored) 
        leds[_cLength-1-position] = color[i];
       leds[_cLength-position] = color[i];
    }
    delay(20);
